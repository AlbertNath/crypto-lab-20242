...
// Crear una Cookie de manera aleatoria
...
if (isset($_POST['outputCookie'])) {
  if(isset($_COOKIE['authKey'])) {
    echo "<h2 align='center'> authKey Cookie: ".sanitize_input($_COOKIE['authKey'])."</h2>";
  } else {
...
if (isset($_GET['name'])) {
  ...
  echo "<p align='center'>Hola ".$name."! Bienvenido!</p>";
...
if (isset($_POST['comentario'])) {
  ...
  $comentario = mysqli_real_escape_string($link, $comentario); // Escapar la entrada
  $insert_sql = "INSERT INTO comentarios (comentario) VALUES ('$comentario')";
...
if ($result->num_rows > 0) {
  while($row = $result->fetch_assoc()) {
    echo "<tr><td style='width:35%;padding:10px'>Comentario<br /><hr />".sanitize_input($row["comentario"])."<br /></td></tr>";
  }
} else {
